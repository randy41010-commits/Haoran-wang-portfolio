...
function Track({ no, title, url }: { no: string; title: string; url: string }) {
  ...
  return <article className="music-item"><div className="soundcloud-player"><iframe title={`${title} — SoundCloud`} src={embedUrl} allow="autoplay" loading="lazy" /></div><div className="music-info"><span>{no} / SOUND</span><h2>{title}</h2><a href={url} target="_blank" rel="noreferrer">Open on SoundCloud ↗</a></div></article>;
}
export default function MusicPage() {
  return <main className="site-frame" id="top"><SiteHeader /><PageIntro index="03" title="Music" text="Compositions and recorded works." /><section className="music-list" aria-label="Music works"><h2 className="music-category">Traditional Composition</h2>{traditionalCompositions.map((track) => <Track key={track.no} {...track} />)}<h2 className="music-category music-category-secondary">Other Music</h2>{otherMusic.map((track) => <Track key={track.no} {...track} />)}</section><SiteFooter /></main>;
}